...
public class PrintingMachine {
    ...
    public void printEdition(Edition edition, boolean isColor) throws UnsupportedColorException, IllegalArgumentException {
        if (isColor && !colorSupport) {
            throw new UnsupportedColorException("The machine does not support color printing.");
        }
        int totalSheetsRequired = edition.getNumberOfPages();
        if (currentPaperLoad < totalSheetsRequired) {
            throw new IllegalArgumentException("There isn't enough paper to print the publication.");
        }
        double printingTime = calculatePrintingTime(totalSheetsRequired);
        System.out.printf("The printing will take approximately %.2f minutes.%n", printingTime);
        ...
        System.out.println("Printed edition: " + edition.getTitle() + ". Total copies: " + editionCopies.get(edition));
    }
